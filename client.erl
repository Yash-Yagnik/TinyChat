...
do_join(State, Ref, ChatName) ->
	User_chatrooms = State#cl_st.con_ch,
	case maps:find(ChatName, User_chatrooms) of
		{ok, _PID} ->
			{err, State};
		error ->
			whereis(server)!{self(), Ref, join, ChatName},
			receive
			  {ChatPID, _R, connect, History} ->
				{History, State#cl_st{con_ch = maps:put(ChatName, ChatPID, User_chatrooms)}}
			end
	end.
...
do_leave(State, Ref, ChatName) ->
	...
	case maps:find(ChatName, User_chatrooms) of
	  {ok, _PID} -> 
		...
		receive
		  {_SPID, _R, ack_leave} ->
			{ok, State#cl_st{con_ch = maps:remove(ChatName, User_chatrooms)}}
		end;
	  error ->
		{err, State}
	  end.
